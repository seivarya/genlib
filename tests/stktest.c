...
static void test_empty_operations(void) {
	...
	CHECK(is_empty(stk) == 1);
	CHECK(peek(stk) == NULL);
	pop(stk); /* no-op on empty, should not crash */
	stack_destruct(stk);
}
...
static void test_lifo_order(void) {
	...
	CHECK(*(int *)peek(stk) == 3);
	pop(stk);
	CHECK(*(int *)peek(stk) == 2);
	pop(stk);
	CHECK(*(int *)peek(stk) == 1);
	pop(stk);
	CHECK(stk->length == 0);
	CHECK(is_empty(stk) == 1);
	CHECK(peek(stk) == NULL);
...
static void test_multiple_push_pop(void) {
	...
	for (int i = 0; i < 20; i++)
		vals[i] = i;
	for (int i = 0; i < 20; i++)
		push(stk, &vals[i], &TD_INT);
	CHECK(stk->length == 20);
	for (int i = 19; i >= 0; i--) {
		CHECK(*(int *)peek(stk) == i);
		pop(stk);
	}
	CHECK(stk->length == 0);
	CHECK(is_empty(stk) == 1);
